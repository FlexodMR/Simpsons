...
const char ApplicationName[] = "Physics Coverage Test";
...
{
    // Create a window
    const char appName[] = "Physics Coverage Application";
...
LRESULT Win32App::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
        ...
        case WM_MOUSEWHEEL:
            ...
            GetApplication()->GetRenderer()->Zoom( static_cast<float>( (short) HIWORD(wParam) / 4 ) );
        break;
        ...
        case WM_CHAR:
        {
            switch(wParam)
            {
                ...
                case 'n':
                case 'N':
                    GetApplication()->GetRenderer()->NextObjectType();
                break;

                case 's':
                case 'S':
                    GetApplication()->GetRenderer()->ToggleDisplaySimStat();
                break;
