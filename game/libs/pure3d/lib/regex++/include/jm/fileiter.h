...
#define FI_WIN32_MAP
#define FI_POSIX_DIR
#elif (defined(__WIN32__) || defined(_WIN32) || defined(WIN32)) && !defined(JM_NO_WIN32)
#define FI_WIN32_MAP
#define FI_WIN32_DIR
#else
#define FI_POSIX_MAP
...
#endif
#else
#error fileiter.h needs either POSIX or WIN32 directory functions...
#endif
#include <assert.h>
#include <dirent.h>
...
#elif !defined(JM_NO_STL)  // use C API to emulate the memory map:
...
class JM_IX_DECL mapfile
{
   ...
   pointer* _first;
   pointer* _last;
   mutable JM_STD::list<pointer*> condemed;
   enum sizes
   {
      buf_size = 4096
   };
...
class JM_IX_DECL file_iterator : public JM_INPUT_ITERATOR(const char*, JM_STDC::ptrdiff_t)
{
   char* _root;
   char* _path;
   char* ptr;
...
class JM_IX_DECL directory_iterator : public JM_INPUT_ITERATOR(const char*, JM_STDC::ptrdiff_t)
{
   char* _root;
   char* _path;
   char* ptr;
...
using JM::directory_iterator;
using JM::file_iterator;
using JM::mapfile;
