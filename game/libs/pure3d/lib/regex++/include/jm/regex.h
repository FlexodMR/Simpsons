...
class JM_IX_DECL bad_expression : public JM_STD::exception
{
#ifdef RE_LOCALE_CPP
   JM_STD::string code;
public:
   bad_expression(const JM_STD::string& s) : code(s) {}
#else
   unsigned int code;
public:
   bad_expression(unsigned int err) : code(err) {}
#endif
   bad_expression(const bad_expression& e) : JM_STD::exception(e), code(e.code) {}
   bad_expression& operator=(const bad_expression& e)
   {
      #ifdef JM_MSC_VER
      ((JM_STD::exception*)(this))->operator=(e);
      #else
      JM_STD::exception::operator=(e);
      #endif
      code = e.code;
      return *this;
   }
...
public:
   ...
   {
      #ifdef RE_LOCALE_CPP
      return icase ? JM_USE_FACET(l, JM_STD::ctype<char>).tolower((char_type)c) : c;
      #else
   ...
   )
   {
      ...
      re_transform(out, in);
#else
      out = JM_USE_FACET(l, JM_STD::collate<char>).transform(in.c_str(), in.c_str() + in.size()).c_str();
#endif
   }
...
#if defined(RE_LOCALE_CPP)
   static bool RE_CALL is_class(char c, jm_uintfast32_t f, const JM_STD::locale& l)
   {
      if(JM_USE_FACET(l, JM_STD::ctype<char>).is((JM_STD::ctype<char>::mask)(f & char_class_all_base), c))
         return true;
...
public:
   ...
   {
      #ifdef RE_LOCALE_CPP
      return icase ? JM_USE_FACET(l, JM_STD::ctype<wchar_t>).tolower((char_type)c) : c;
      #else
   ...
   )
   {
      ...
      re_transform(out, in);
#else
      out = JM_USE_FACET(l, JM_STD::collate<wchar_t>).transform(in.c_str(), in.c_str() + in.size()).c_str();
#endif
   }
...
#if defined(RE_LOCALE_CPP)
   static bool RE_CALL is_class(wchar_t c, jm_uintfast32_t f, const JM_STD::locale& l)
   {
      if(JM_USE_FACET(l, JM_STD::ctype<wchar_t>).is((JM_STD::ctype<wchar_t>::mask)(f & char_class_all_base), c))
         return true;
      if((f & char_class_underscore) && (c == '_'))
         return true;
      if((f & char_class_blank) && ((c == ' ') || (c == '\t')))
...
public:
   ...
   {
      #ifdef RE_LOCALE_CPP
      return JM_USE_FACET(l, JM_STD::ctype<char>).tolower((char_type)c);
      #else
...
public:
   ...
   {
      #ifdef RE_LOCALE_CPP
      return JM_USE_FACET(l, JM_STD::ctype<wchar_t>).tolower((char_type)c);
      #else
...
public:
...
#ifdef RE_LOCALE_CPP
   JM_STD::string RE_CALL errmsg()const
   {
   ...
   #ifdef RE_LOCALE_CPP
   JM_STD::locale RE_CALL imbue(const JM_STD::locale& l);
   
   const JM_STD::locale& RE_CALL locale()const
   {
      return locale_inst;
   }
   #endif
...
public:
   ...
   typedef JM_MAYBE_TYPENAME REBIND_TYPE(charT, alloc_type) c_alloc_type;
   typedef typename c_alloc_type::size_type size_type;
...
private:
   ...
   void RE_CALL compile_map(re_syntax_base* node, unsigned char* _map, unsigned int* pnull, unsigned char mask, re_syntax_base* terminal = NULL)const;
   ...
   re_syntax_base* RE_CALL compile_set_aux(jstack<re_str<charT>, Allocator>& singles, jstack<re_str<charT>, Allocator>& ranges, jstack<jm_uintfast32_t, Allocator>& classes, jstack<re_str<charT>, Allocator>& equivalents, bool isnot, const _narrow_type&);
   re_syntax_base* RE_CALL compile_set_aux(jstack<re_str<charT>, Allocator>& singles, jstack<re_str<charT>, Allocator>& ranges, jstack<jm_uintfast32_t, Allocator>& classes, jstack<re_str<charT>, Allocator>& equivalents, bool isnot, const _wide_type&);
...
public:
...
#ifndef JM_AIX
   reg_expression(const charT* p, size_type len, jm_uintfast32_t f, const Allocator& a);
#else
   reg_expression(const charT* p, size_type len, jm_uintfast32_t f, const Allocator& a)
       : data(a), pkmp(0)
   {
...
#endif
   reg_expression(const reg_expression<charT, traits, Allocator>&);
   ~reg_expression();
   reg_expression& RE_CALL operator=(const reg_expression<charT, traits, Allocator>&);

#ifndef JM_NO_MEMBER_TEMPLATES

   template <class ST, class SA>
   unsigned int RE_CALL set_expression(const JM_STD::basic_string<charT, ST, SA>& p, jm_uintfast32_t f = regbase::normal)
   { return set_expression(p.data(), p.data() + p.size(), f); }

   template <class ST, class SA>
   reg_expression(const JM_STD::basic_string<charT, ST, SA>& p, jm_uintfast32_t f = regbase::normal, const Allocator& a = Allocator())
    : data(a), pkmp(0) { set_expression(p, f); }

#elif !defined(JM_NO_STRING_DEF_ARGS)
   unsigned int RE_CALL set_expression(const JM_STD::basic_string<charT>& p, jm_uintfast32_t f = regbase::normal)
   { return set_expression(p.data(), p.data() + p.size(), f); }

   reg_expression(const JM_STD::basic_string<charT>& p, jm_uintfast32_t f = regbase::normal, const Allocator& a = Allocator())
    : data(a), pkmp(0) { set_expression(p, f); }

#endif


   bool RE_CALL operator==(const reg_expression<charT, traits, Allocator>&)const;
   bool RE_CALL operator<(const reg_expression<charT, traits, Allocator>&)const;
   ...
   //
   // egcs seems not to be able to cope with template friend
   // functions if these try to call const member functions,
   ...
   friend bool query_match_aux(I first, I last, reg_match<I, A>& m, const reg_expression<charT, traits, A2>& e, 
                        unsigned flags, _priv_match_data<I, A>& pd, I* restart);
...
private:
   ...
   static bool RE_CALL can_start(charT c, const unsigned char* _map, unsigned char mask, const _wide_type&);
   static bool RE_CALL can_start(charT c, const unsigned char* _map, unsigned char mask, const _narrow_type&);
};
...
struct sub_match
{
   ...
   operator JM_STD::basic_string<charT, traits, Allocator> ()const;
#elif !defined(JM_NO_STRING_DEF_ARGS)
   operator JM_STD::basic_string<char> ()const;
#ifndef JM_NO_WCSTRING
   operator JM_STD::basic_string<wchar_t> ()const;
#endif
#endif
   operator int()const;
   ...
   bool operator==(const sub_match<iterator>& that)const
   {
      return (first == that.first) && (second == that.second) && (matched == that.matched);
   }
   bool RE_CALL operator !=(const sub_match<iterator>& that)const
   { return !(*this == that); }

   size_t RE_CALL length()const
   {
      size_t n = 0;
      JM_DISTANCE((iterator)first, (iterator)second, n);
      return n;
   }
};
...
template <class iterator>
template <class charT, class traits, class Allocator>
sub_match<iterator>::operator JM_STD::basic_string<charT, traits, Allocator> ()const
{
#if !defined(JM_NO_EXCEPTIONS) && !defined(JM_NO_TYPEINFO)
   if(typeid(charT) != typeid(*first))
      throw JM_STD::bad_cast();
#endif
   JM_STD::basic_string<charT, traits, Allocator> result;
   iterator i = first;
   while(i != second)
   {
      result.append(1, *i);
...
#elif !defined(JM_NO_STRING_DEF_ARGS)
template <class iterator>
sub_match<iterator>::operator JM_STD::basic_string<char> ()const
{
#if !defined(JM_NO_EXCEPTIONS) && !defined(JM_NO_TYPEINFO)
   if(typeid(char) != typeid(*first))
      throw JM_STD::bad_cast();
#endif
   JM_STD::basic_string<char> result;
   iterator i = first;
   while(i != second)
   {
      result.append(1, *i);
...
#ifndef JM_NO_WCSTRING
template <class iterator>
sub_match<iterator>::operator JM_STD::basic_string<wchar_t> ()const
{
#if !defined(JM_NO_EXCEPTIONS) && !defined(JM_NO_TYPEINFO)
   if(typeid(wchar_t) != typeid(*first))
      throw JM_STD::bad_cast();
#endif
   JM_STD::basic_string<wchar_t> result;
   iterator i = first;
   while(i != second)
   {
      result.append(1, *i);
...
sub_match<iterator>::operator int()const
{
   ...
   neg *= (int)re_toi(i, j, 10 MAYBE_PASS_LOCALE(JM_STD::locale()));
#if !defined(JM_NO_EXCEPTIONS) && !defined(JM_NO_TYPEINFO)
   if(i != j)
   {
      throw JM_STD::bad_cast();
...
{
   iterator i = first;
   unsigned int result = (int)re_toi(i, iterator(second), 10 MAYBE_PASS_LOCALE(JM_STD::locale()));
#if !defined(JM_NO_EXCEPTIONS) && !defined(JM_NO_TYPEINFO)
   if(i != second)
   {
      throw JM_STD::bad_cast();
...
public:
   typedef Allocator alloc_type;
   typedef JM_MAYBE_TYPENAME REBIND_TYPE(iterator, Allocator) it_alloc_type;
   typedef typename it_alloc_type::size_type size_type;
...
public:
   ...
   reg_match_base(const reg_match_base<iterator, Allocator>& m)
   {
      ref = m.ref;
      ++(ref->count);
   }

   reg_match_base& RE_CALL operator=(const reg_match_base<iterator, Allocator>& m);
   ...
   size_t RE_CALL length(unsigned int sub = 0)const
   {
      ...
      if(m.matched == false)
         return 0;
      JM_DISTANCE((iterator)m.first, (iterator)m.second, n);
      return n;
   }
   ...
   unsigned int RE_CALL position(unsigned int sub = 0)const
   {
      jm_assert(ref->cmatches);
      size_t n = 0;
      ...
      if(s.matched == false)
         return -1;
      JM_DISTANCE((iterator)ref->base, (iterator)s.first, n);
      return n;
   }
   ...
   void swap(reg_match_base<iterator, Allocator>& that)
   {
   ...
   bool operator==(const reg_match_base<iterator, Allocator>& that)const;
   bool operator<(const reg_match_base<iterator, Allocator>& that)const
...
private:
   ...
   void RE_CALL maybe_assign(const reg_match_base<iterator, Allocator>& m);
   void RE_CALL init_fail(iterator i, iterator j);

   void RE_CALL set_first(iterator i);
   void RE_CALL set_first(iterator i, size_type pos);
   ...
   void RE_CALL set_second(iterator i, size_type pos)
   {
      cow();
   ...
   void RE_CALL set_base(iterator pos)
   {
      ref->base = pos;
   }
};
...
template <class iterator, class Allocator>
void RE_CALL reg_match_base<iterator, Allocator>::set_first(iterator i)
{
   cow();
   ref->head.second = i;
...
template <class iterator, class Allocator>
void RE_CALL reg_match_base<iterator, Allocator>::set_first(iterator i, typename reg_match_base<iterator, Allocator>::size_type pos)
{
   cow();
...
template <class iterator, class Allocator>
bool reg_match_base<iterator, Allocator>::operator==(const reg_match_base<iterator, Allocator>& that)const
{
   if(*ref != *(that.ref))
      return false;
...
template <class iterator, class Allocator>
void RE_CALL reg_match_base<iterator, Allocator>::set_size(typename reg_match_base<iterator, Allocator>::size_type n)
{
   if(ref->cmatches != n)
   {
...
template <class iterator, class Allocator>
void RE_CALL reg_match_base<iterator, Allocator>::set_size(typename reg_match_base<iterator, Allocator>::size_type n, iterator i, iterator j)
{
   if(ref->cmatches != n)
   {
      c_reference* newref = (c_reference*)ref->allocate(sizeof(sub_match<iterator>) * n + sizeof(c_reference));;
...
public:
   ...
   reg_match(const reg_match<iterator, Allocator>& m);
   reg_match& operator=(const reg_match<iterator, Allocator>& m);
   /*
   bool operator==(const reg_match& that)const
   { return reg_match_base<iterator, Allocator>::operator==(that); }
   bool operator<(const reg_match& that) const
   { return position() < that.position(); }
   */
};
...
{
   typedef typename reg_match_base<iterator, Allocator>::c_reference c_ref;
   reg_match_base<iterator, Allocator>::ref = (c_ref *)m.ref->allocate(sizeof(sub_match<iterator>) * m.ref->cmatches + sizeof(c_ref));
#ifndef JM_NO_EXCEPTIONS
   try{
#endif
      new (reg_match_base<iterator, Allocator>::ref) c_ref(*m.ref);
   ...
   { 
      m.ref->deallocate((char*)(void*)reg_match_base<iterator, Allocator>::ref, sizeof(sub_match<iterator>) * m.ref->cmatches + sizeof(c_ref));
      throw; 
   }
#endif
}
...
#ifndef JM_NO_EXCEPTIONS
using JM::bad_expression;
#endif
using JM::char_regex_traits;
using JM::char_regex_traits_i;
using JM::regbase;
using JM::reg_expression;
using JM::reg_match;
using JM::reg_match_base;
using JM::sub_match;
using JM::regex;
using JM::cmatch;
#ifndef JM_NO_WCSTRING
...
using JM::wcmatch;
#endif
using JM::query_match;
using JM::reg_search;
using JM::reg_grep;
using JM::reg_format;
using JM::reg_merge;
using JM::jm_def_alloc;
