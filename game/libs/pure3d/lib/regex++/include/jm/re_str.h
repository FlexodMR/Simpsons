...
public:
   ...
   ~re_str();
   re_str(const re_str<charT>& other);
   re_str(const charT* p1);
   re_str(const charT* p1, const charT* p2);
   re_str(charT c);

   re_str<charT>& RE_CALL operator=(const re_str<charT>& other)
   {
      re_strfree(buf);
      buf = re_strdup(other.buf);
      return *this;
   }
   re_str<charT>& RE_CALL operator=(const charT* p)
   {
      re_strfree(buf);
      buf = re_strdup(p);
      return *this;
   }
   re_str<charT>& RE_CALL operator=(charT c)
   {
      re_strfree(buf);
      buf = re_strdup(&c, &c+1);
      return *this;
   ...
   bool RE_CALL operator==(const re_str<charT>& other)const { return re_strcmp(buf, other.buf) == 0; }
