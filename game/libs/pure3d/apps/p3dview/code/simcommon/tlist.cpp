...
void VoidPList::ResizeArray(int newarraysize)
{
    ...
    {
        voidp *array = new voidp[newarraysize];
...
ListLocker::ListLocker(VoidPList* list)
    : mList(list)
{
    P3DASSERT(mList != NULL);
    P3DASSERTMSG(mList->mLock == false, "List already locked, can't apply successive locks", "ListLocker");
    mList->mLock = true;
}
