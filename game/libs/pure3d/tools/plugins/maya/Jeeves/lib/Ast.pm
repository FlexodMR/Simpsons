...
my @defined_values_stack;
sub visit {
    no strict 'refs';
    my $this = shift;
    package main;
    my ($var, $val, $old_val, %saved_values, %defined_values);
    while (($var,$val) = each %{$this}) {
        $defined_values{$var} = 1;        
    ...
    push (@defined_values_stack, \%defined_values);
}

sub bye {
    my $rh_saved_values = pop(@saved_values_stack);
    my $rh_defined_values = pop(@defined_values_stack);
    no strict 'refs';
    package main;
    my ($var,$val);
    while (($var,$val) = each %$rh_defined_values) {
        undef $$var;
    }
