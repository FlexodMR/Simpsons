...
MObject  ${shadername}::aUVCoord;
MObject  ${shadername}::aUCoord;
MObject  ${shadername}::aVCoord;
...
@foreach texture_list
    @if( !defined $texture_attribute || $texture_attribute eq "true" )
    m${texture_name}Image = NULL;
    m${texture_name}ImageDataOriginal = NULL;
    m${texture_name}ImageData = NULL;
    m${texture_name}ImageDataSize = 0;
    m${texture_name}LastBlend[0] = -1.0F;
    m${texture_name}LastBlend[1] = -1.0F;
    m${texture_name}LastBlend[2] = -1.0F;
    m${texture_name}LastBlend[3] =  1.0F;
    @endif
@end

    glGenTextures( NUM_${shadername}_TEXTURE_OBJECTS, mTextureObjects );
}
...
${shadername}::~$shadername()
{
    glDeleteTextures( NUM_${shadername}_TEXTURE_OBJECTS, mTextureObjects );

@foreach texture_list
    @if( !defined $texture_attribute || $texture_attribute eq "true" )
    delete m${texture_name}Image;
    m${texture_name}Image = NULL;
    @endif
@end
}
...
MStatus ${shadername}::initialize()
{
    ...
    // Maya uses the following .uvCoord attribute internally for HW texture mapping.
    // See Maya SDK Documentation: Appendix C: Rendering attributes.

    aUCoord = nAttr.create( "uCoord", "u", MFnNumericData::kFloat, 0.0 );
    nAttr.setStorable(false);
    nAttr.setWritable(true);
    nAttr.setRenderSource(true);

    aVCoord = nAttr.create( "vCoord", "v", MFnNumericData::kFloat, 0.0 );
    nAttr.setStorable(false);
    nAttr.setWritable(true);
    nAttr.setRenderSource(true);

    aUVCoord = nAttr.create( "uvCoord", "uv", aUCoord, aVCoord );
    nAttr.setStorable(false);
    nAttr.setWritable(true);
    nAttr.setRenderSource(true);
    nAttr.setStorable(false);
    ...
    addAttribute( aUCoord );
    addAttribute( aVCoord );
    ...
    attributeAffects ( aUCoord, aOutColor);
    attributeAffects ( aVCoord, aOutColor);
...
MStatus ${shadername}::getFloat(MObject attr, float& value)
{
    MStatus status;
    // Get the attr to use
    //
    ...
    if (!status)
    {
        status.perror("simpleHwShader::bind plug.getValue.");
        return status;
    }
...
MStatus ${shadername}::getFloat3(MObject attr, float value[3])
{
    MStatus status;
    // Get the attr to use
    //
    ...
    if (!status)
    {
        status.perror("simpleHwShader::bind construct data.");
        return status;
    }
    ...
    if (!status)
    {
        status.perror("simpleHwShader::bind get values.");
        return status;
    }
...
@foreach texture_list
    @if( !defined $texture_attribute || $texture_attribute eq "true" )
//===========================================================================
// ${shadername}::Bind${texture_name}
//===========================================================================
// Description: Determines if the ${texture_name} channel has been textured
//              and has a .fileTextureName. If so, a new tlImage and 
//              ImageData are generated.
//
// Constraints: 
//
//  Parameters: 
...
//
//===========================================================================
bool ${shadername}::Bind${texture_name}( void )
{
    ...
    MPlug evalPlug( thisMObject(), a${texture_name} );

    float blend[3];
    if ( getFloat3( a${texture_name}Blend, blend ) != MS::kSuccess ) 
    {
        blend[0] = blend[1] = blend[2] = 0.5;
    }

    if ( evalPlug.isConnected() )
    {
        MPlugArray connections;
        evalPlug.connectedTo( connections, true /* asDst */, false, &status );
        MFnDependencyNode       fnDependNode( connections[0].node(), &status );
        if ( status == MS::kSuccess )
        {
            MPlug filePlug = fnDependNode.findPlug( "fileTextureName", &status );
            if ( status == MS::kSuccess )
            {
                bValid = true;

                MString newValue;
                filePlug.getValue( newValue );

                if ( newValue != mCurrent${texture_name} )
                {
                    mCurrent${texture_name} = newValue;

                // ----------------  Unbind the Texture from OpenGL and delete data  ----------------

                    Flush${texture_name}();
                    bValid = false;

                // ----------------  Generate and allocate a new texture  --------------------

                    m${texture_name}Image = new tlImage;

                // ----------------  Bind the texture to OpenGL  --------------------

                    bool bLoaded = m${texture_name}Image->Load( mCurrent${texture_name}.asChar() );

                    if ( bLoaded )
                    {
                        m${texture_name}Image->Convert( 32, false, true, false );

                        bValid = true;
                        // Force blend to update
                        m${texture_name}LastBlend[0] = -1.0F;

                        int bytesPerPixel = 3;
                        if ( m${texture_name}Image->GetBpp() == 32 ) bytesPerPixel = 4;

                        int pixelrow = m${texture_name}Image->GetWidth() * bytesPerPixel;
                        int pixels = m${texture_name}Image->GetHeight() * pixelrow;

                        // Allocate TWO copies of the image.
                        // The Original holds that directly copied from the original tlImage source.
                        // The other is a buffer into which modulated pixels will be written,
                        // using the blend colour.
                        delete [] m${texture_name}ImageDataOriginal;
                        delete [] m${texture_name}ImageData;
                        m${texture_name}ImageDataOriginal = new unsigned char [ pixels ];
                        m${texture_name}ImageData         = new unsigned char [ pixels ];
                        m${texture_name}ImageDataSize = pixels;

                        // Pixel data must be copied row-by-row.
                        unsigned char** pixeldata = m${texture_name}Image->GetPixels();

                        int w = m${texture_name}Image->GetWidth() * bytesPerPixel;
                        int h;

                        unsigned char* ptrA = m${texture_name}ImageDataOriginal;
                        unsigned char* ptrB = m${texture_name}ImageData;
                        for ( h = 0; h < m${texture_name}Image->GetHeight(); h++ )
                        {
                            memcpy( ptrA, pixeldata[h], pixelrow );
                            memcpy( ptrB, pixeldata[h], pixelrow );
                            ptrA += pixelrow;
                            ptrB += pixelrow;
                        }
                ...
                }       // If require new image
            }       // If .fileTextureName exists
        }
    }
    ...
    {
        Flush${texture_name}();
        mCurrent${texture_name} = "";
    }

    glBindTexture( GL_TEXTURE_2D, mTextureObjects[ ${texture_name}_TEXTURE_OBJECT ] );
...
// *** Don't use GL_NEAREST_MIPMAP_NEAREST when you aren't building mipmaps!!!
//    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);

    if ( m${texture_name}ImageData != NULL )
    {
        int rc = 0;
        if ( m${texture_name}Image->GetBpp() == 32 ) 
        {
            if ( ( blend[0] != m${texture_name}LastBlend[0] ) || ( blend[1] != m${texture_name}LastBlend[1] ) || ( blend[2] != m${texture_name}LastBlend[2] ) )
            {
                memcpy( m${texture_name}ImageData, m${texture_name}ImageDataOriginal, m${texture_name}ImageDataSize );

                unsigned int pixels = m${texture_name}ImageDataSize / 4;
                unsigned char* p = m${texture_name}ImageData;
                for ( unsigned int b = 0; b < pixels; b++ )
                {
                    p[0] = (unsigned char)( p[0] * blend[2] );    // colour.BlueCh();
                    p[1] = (unsigned char)( p[1] * blend[1] );    // colour.GreenCh();
                    p[2] = (unsigned char)( p[2] * blend[0] );    // colour.RedCh();
                    p[3] = 255;    // (unsigned char)( colour.Luminosity() * 255 );
                    p += 4;
                }

                m${texture_name}LastBlend[0] = blend[0];
                m${texture_name}LastBlend[1] = blend[1];
                m${texture_name}LastBlend[2] = blend[2];
            }

            glTexImage2D( GL_TEXTURE_2D, 0, 4, m${texture_name}Image->GetWidth(), m${texture_name}Image->GetHeight(), 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, m${texture_name}ImageData);
//            rc = gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA8, mColorImage->GetWidth(), mColorImage->GetHeight(), GL_BGRA_EXT, GL_UNSIGNED_BYTE, mColorImageData );
        }
        else
        {
            glTexImage2D( GL_TEXTURE_2D, 0, 3, m${texture_name}Image->GetWidth(), m${texture_name}Image->GetHeight(), 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, m${texture_name}ImageData );
//            rc = gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB8, mColorImage->GetWidth(), mColorImage->GetHeight(), GL_BGR_EXT, GL_UNSIGNED_BYTE, mColorImageData );
        }
    ...
    {
        glTexImage2D( GL_TEXTURE_2D, 0, 3, 0, 0, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, m${texture_name}ImageData );
    }

    return bBindSuccessful;
}

bool ${shadername}::${texture_name}HasAlpha( void ) const
{
    return m${texture_name}Image->HasAlpha();
}

void ${shadername}::Flush${texture_name}( void )
{
// ----------------  Flush the cached Texture  ----------------

//    glDeleteTextures( 1, (GLuint *)&(GLX_TEXTURE_2D_${texture_name}) );
    delete m${texture_name}Image;
    m${texture_name}Image = NULL;
