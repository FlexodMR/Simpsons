    ...
    Files( )
{
    ...
    static struct option long_options[] = {
      ...
      {"library-path",required_argument,NULL,0},
      {"duplicate",no_argument,NULL,0},
      {"help",no_argument,NULL,0},
      {0,0,0,0}
    };
    while ((ch = getopt_long(argc, argv, "?hHvVRP:no:l:d",long_options,&option_index)) != EOF)
    {
        switch(ch)
        {
...
Parameters::Usage()
{
    ...
    puts( "\nCopyright Radical Entertainment 2015\n\n"
         ...
         "-l, --library-path path  Specify the path to the library files\n"
         "-d, --duplicate          Enable duplication of geometry instead of\n"
         "                          instances\n"
            "\n"
            "Standard options:\n"
...
Parameters::HistoryChunk()
{
    tlHistory history;
    char buf[512];
    sprintf(buf,"Instancer version %s (with ATG %s)", version, ATG_VERSION);
    history.AddLine(buf);
    char* b = buf;
    int i;
    for( i = 0; i < Argc; i++)
