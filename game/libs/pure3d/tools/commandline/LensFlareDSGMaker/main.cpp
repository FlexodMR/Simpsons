...
int main(int argc, char* argv[])
{
    ...
    for(int i = 0; i < Param->Files.Count(); i++) 
    {
		...
		tlLensFlareDSGChunk* lensFlareDSG = new tlLensFlareDSGChunk;
		lensFlareDSG->SetNumBillBoardQuads( 0 );
        int ch;
        ...
        for(ch=0; ch < inchunk->SubChunkCount(); ch++)
        {
            ...
            switch(sub->ID())
            {
				case P3D_COMPOSITE_DRAWABLE:
					{
						lensFlareDSG->AppendSubChunk(sub,FALSE);
						lensFlareDSG->SetName( sub->GetName() );
					}
					break;	
				case Pure3D::BillboardObject::QUAD_GROUP:
					{
						lensFlareDSG->AppendSubChunk(sub,FALSE);
	                    lensFlareDSG->SetNumBillBoardQuads(lensFlareDSG->GetNumBillBoardQuads()+1);
					}
					break;
					// Vis testing mesh
		...
		outchunk->AppendSubChunk(lensFlareDSG, FALSE);
