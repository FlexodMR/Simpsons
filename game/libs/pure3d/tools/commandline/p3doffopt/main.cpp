/*===========================================================================
    ...
    This tool turn off optimization of geometry with given name
        ...
        for(ch=0; ch < inchunk->SubChunkCount(); ch++)
        {
            ...
            switch(sub->ID())
            {
                ...
                case Pure3D::Mesh::MESH:
                {
                    ...
                    {               
                          tlDataChunk *ch = pgMesh->Chunk( );
                          tlMeshChunk *mch = dynamic_cast< tlMeshChunk *>( ch );
                          mch->SetVersion( GEOMETRY_NONOPTIMIZE_VERSION );
                          outchunk->AppendSubChunk( mch );
                     }
                     else
