    ...
    Files( )
{
    ...
    static struct option long_options[] = {
      ...
      {"crosscheck",no_argument,NULL,0},
      {"tolerance",required_argument,NULL,0},
      {"number",required_argument,NULL,0},
      {"help",no_argument,NULL,0},
      {0,0,0,0}
    };
    while ((ch = getopt_long(argc, argv, "?hHvVRP:no:ct:b:",long_options,&option_index)) != EOF)
    {
        switch(ch)
        {
...
{
    printf("roadOptimizer version %s\n",version);
    printf("Using ATG %s Changelist %s %s\n", ATG_VERSION, compileChangelist, compileTime);
    puts( "\nCopyright Radical Entertainment 2015\n\n"
            "   Usage: roadOptimizer [options] <inputfile> [<inputfile> ...]\n\n"

         "This tool optimizes road data\n"
            "\n"
            "Try roadOptimizer --help for more info.\n");
    exit(1);
}
...
Parameters::Usage()
{
    ...
    puts( "\nCopyright Radical Entertainment 2015\n\n"
         ...
         "-c, --crosscheck        update multiple files that may contain shared \n"
         "                        intersections.\n"
         "-t, --tolerance         How close can the intersections be?\n"
         "-b, --number            The N first files will be compared to all the \n"
         "                        others.\n"
            "\n"
            "Standard options:\n"
...
Parameters::HistoryChunk()
{
    tlHistory history;
    char buf[512];
    sprintf(buf,"roadOptimizer version %s (with ATG %s)", version, ATG_VERSION);
    history.AddLine(buf);
    char* b = buf;
    int i;
    for( i = 0; i < Argc; i++)
