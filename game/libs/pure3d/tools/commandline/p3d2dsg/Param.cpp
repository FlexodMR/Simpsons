    ...
    Files( )
{
    ...
    static struct option long_options[] = {
      ...
      {"intersects",no_argument,NULL,0},
      {"worldsphere",no_argument,NULL,0},
      {"help",no_argument,NULL,0},
      {0,0,0,0}
    };
    while ((ch = getopt_long(argc, argv, "?hHvVRP:no:iw",long_options,&option_index)) != EOF)
    {
        switch(ch)
        {
...
{
    printf("p3d2dsg version %s\n",version);
    printf("Using ATG %s Changelist %s %s\n", ATG_VERSION, compileChangelist, compileTime);
    puts( "\nCopyright Radical Entertainment 2015\n\n"
            "   Usage: p3d2dsg [options] <inputfile> [<inputfile> ...]\n\n"

         "This tool converts all chunks to their DSG equivalents for use in the AABB tree\n"
         " created/maintained by Devin M. for SRR2\n"
            "\n"
            "Try p3d2dsg --help for more info.\n");
    exit(1);
}
...
Parameters::Usage()
{
    ...
    puts( "\nCopyright Radical Entertainment 2015\n\n"
         ...
         "-i, --intersects        Generate IntersectDSG wrappers for all meshes\n"
         "-w, --worldsphere       Generate a single WorldSphere out of the\n"
         "                         contents\n"
            "\n"
            "Standard options:\n"
...
Parameters::HistoryChunk()
{
    tlHistory history;
    char buf[512];
    sprintf(buf,"p3d2dsg version %s (with ATG %s)", version, ATG_VERSION);
    history.AddLine(buf);
    char* b = buf;
    int i;
    for( i = 0; i < Argc; i++)
