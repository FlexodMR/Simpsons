/*===========================================================================
    ...
    This tool converts indexed prims to non-indexed prims for the PS2.
...
int main(int argc, char* argv[])
{
    ...
    for(int i = 0; i < Param->Files.Count(); i++) 
    {
        ...
        if( bVertexAnimed ){        //de-Deindex all vertex-animated meshes first, since vertex
        ...
        for(ch=0; ch < inchunk->SubChunkCount(); ch++)
        {
            ...
            switch(sub->ID())
            {
                ...
                {
                    tlVertexAnimKey *key;
                    key = toollib_find<tlVertexAnimKey>( &inventory, sub->GetName( ) );
                ...
                case Pure3D::Mesh::MESH:
                {
                    ...
                    geo = toollib_find<tlPrimGroupMesh>( &inventory, sub->GetName( ) );
                    if( geo ){
