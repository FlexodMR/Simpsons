/*===========================================================================
    ...
    Wraps up an object into an tlAnimObjDSGWrapperChunk
...
int main(int argc, char* argv[])
{
    ...
    for(int i = 0; i < Param->Files.Count(); i++) 
    {
		...
		tlAnimObjDSGWrapperChunk* pDSGObject = new tlAnimObjDSGWrapperChunk;
        ...
        for(ch=0; ch < inchunk->SubChunkCount(); ch++)
        {
            ...
            case P3D_COMPOSITE_DRAWABLE:
				{
					// Set the name of the DSG object to be 
					// composite drawable name + _Shape
					std::ostringstream oss;
					oss << sub->GetName() << "_Shape";
					pDSGObject->SetName( oss.str().c_str() );
				}
