/*===========================================================================
    ...
    This tool create a copy of a mesh with given name
        ...
        for(ch=0; ch < inchunk->SubChunkCount(); ch++)
        {
            ...
            switch(sub->ID())
            {
                ...
                case Pure3D::Mesh::MESH:
                {
                     ...
                     // check if the skin's name match the given one
                    if((Param->MeshPatterns.Count() == 0) ||
                        glob(&Param->MeshPatterns, pgMesh->GetName()))
                    {               
                         //append the original copy
                          outchunk->AppendSubChunk( pgMesh->Chunk( ) );

                          //create a new copy
                          tlPrimGroupMesh *ms = new tlPrimGroupMesh( sub );
                          char name[ 512 ];
                          name[ 0 ] = 0;

                          //append prefix, and suffix to the name
                          if( Param->Prefix )
                                strcat(  name, Param->Prefix );

                          strcat( name, ms->GetName( ) );

                          if( Param->Suffix )
                                strcat( name, Param->Suffix );

                          ms->SetName( name );
                          outchunk->AppendSubChunk( ms->Chunk( ) );
