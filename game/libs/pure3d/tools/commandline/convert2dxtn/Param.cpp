    ...
    Files( )
{
    ...
    while ((ch = getopt_long(argc, argv, "?hHvVRP:no:f:1q",long_options,&option_index)) != EOF)
    {
        switch(ch)
        {
...
{
    printf("convert2dxtn version %s\n",version);
    printf("Using ATG %s Changelist %s %s\n", ATG_VERSION, compileChangelist, compileTime);
    puts( "\nCopyright Radical Entertainment 2015\n\n"
            "   Usage: convert2dxtn [options] <inputfile> [<inputfile> ...]\n\n"

         "Convert all textures and images of a p3d file into DXTn image format\n"
            "\n"
            "Try convert2dxtn --help for more info.\n");
    exit(1);
}
...
Parameters::Usage()
{
    ...
    puts( "\nCopyright Radical Entertainment 2015\n\n"
         ...
         "-1, --dxt1-only         Only convert to DXT1, if the image needs\n"
         "                         DXT3/5, leave it alone\n"
         "-q, --quick-encode      Quick encode with poor image quality; for\n"
         "                         debugging only\n"
            "\n"
            "Standard options:\n"
...
Parameters::HistoryChunk()
{
    tlHistory history;
    char buf[512];
    sprintf(buf,"convert2dxtn version %s (with ATG %s)", version, ATG_VERSION);
    history.AddLine(buf);
    char* b = buf;
    int i;
    for( i = 0; i < Argc; i++)
