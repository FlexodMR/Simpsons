        ...
        for(ch=0; ch < inchunk->SubChunkCount(); ch++)
        {
            ...
            switch(sub->ID())
            {
                ...
                case Pure3D::Mesh::SKIN:
                {
                    ...
                    {
                        int influences = skin.CountWeights();
                        printf(" before processing %d total weights (%f per vertex)\n",
                                 influences,
                                 (float)influences / (float)skin.NumVertices());
                    }
                    if(Param->MaxInfluences)
                    {
                        skin.MaxWeights(Param->MaxInfluences);
                    }
                    if(Param->InfluenceThreshold != 0.0f)
                    {
                        skin.ThresholdWeights(Param->InfluenceThreshold);
                    }
                    
                    if (Param->OffsetTolerance != 0.0f)
                        skin.TrimOffsets(Param->OffsetTolerance);
                    if(Param->Verbosity>2)
                    {
                        int influences = skin.CountWeights();
                        printf(" after processing %d total weights (%f per vertex)\n",
                                 influences,
                                 (float)influences / (float)skin.NumVertices());
                    }
