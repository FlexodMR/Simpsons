...
///////////////////////////////////////////////////////////////////////////////////////////
const char CoordSubList::smAxis0 = 1;
const char CoordSubList::smAxis1 = 1 << 1;
const char CoordSubList::smAxis2 = 1 << 2;
...
//   int List1Size   = GetWeightedMedian( orPlanePosn, iorAxis );
   ...
   *opList1 = new CoordSubList( mpCellList,                List1Size);
   *opList2 = new CoordSubList( &(mpCellList[List1Size]),  mSize - List1Size);
...
{
   static int sLastAxisChosen = 0;
   Vector3i MedianDeviation, MedianBin;

   assert(ioAxis==SRR_ERR_INDEX);
   ...
   */
      ...
      int nearDeviationThreshold = mWeightedSize/20; //5%

      ioAxis = MedianDeviation.MinIndex();

      if( (ioAxis==sLastAxisChosen) && (ioAxis!=1) )
      {
          int testDeviation = abs(MedianDeviation.mX-MedianDeviation.mZ);
          if( (testDeviation < nearDeviationThreshold)
              || (mSize < 25)             )
          {
              if(ioAxis==0) ioAxis = 2;
              else          ioAxis = 0;
          }
          //else
          //{
          //    printf("Deviation Diff: %d, x:%d, z:%d  size:%d\n", testDeviation, MedianDeviation.mX, MedianDeviation.mZ, mSize );
          //}
      }
      sLastAxisChosen = ioAxis;
...
void CoordSubList::BinCount( int& oMedianDeviation, int& oMedianBin, int iAxis )
{
   ...
   SubArray<int> Bins;
   bool result = theCullCache().Acquire( Bins, DEV_NUM_BINS );
   assert(result);
...
/*            //In the case where they overlap, ensure right ends on GT
  ...
  */
   ...
   if( right->BlockIndex(iAxis) < iMedianBin )
   {
      assert( (right+1)->BlockIndex(iAxis) >= iMedianBin );
      RightIndex++;
   }
...
void CoordSubList::SeekMinPlane( float& oPlanePosn, int i, int iAxis )
{
   ...
   for( i++; i<mSize; i++ )
   {
      assert( mpCellList[i].MinPlane(iAxis) < mpCellList[i].MaxPlane(iAxis) );
...
void CoordSubList::SeekMaxPlane( float& oPlanePosn, int iCur, int iEnd, int iAxis )
{
   ...
   {
      assert( mpCellList[iCur].MinPlane(iAxis) < mpCellList[iCur].MaxPlane(iAxis) );
