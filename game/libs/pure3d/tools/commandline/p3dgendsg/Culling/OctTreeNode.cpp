...
void OctTreeNode::GrowTreeHeuristic(   int            iAxis, 
                                       ...
                                       int            iMinVertexCount )
{
   assert( mpChildLT     == NULL );
   assert( mpChildGT     == NULL );
   assert( mpCoordList   == NULL );

   mpCoordList = ipCoordList;

   if( RoomToGrow( iRecursionsRemaining, iMinVertexCount ) )
   {
      mpChildLT = new OctTreeNode();
      mpChildGT = new OctTreeNode();
      ...
      assert( pListLT->Size() > 0 );
      assert( pListGT->Size() > 0 );
...
)
{
   ...
   if( RoomToGrow( iRecursionsRemaining, iMinVertexCount ) )
   {
      ...
      assert(((int)(mPlanePosn*100.0f))%((int)(irGranularity[mAxis]*100.0f))==0);
