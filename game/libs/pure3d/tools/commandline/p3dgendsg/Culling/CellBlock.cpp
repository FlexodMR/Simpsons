   ...
   for( Index.mX=0; Index.mX<mBlockCoord.mExtents[0]; Index.mX++ )
   {
      for( Index.mY=0; Index.mY<mBlockCoord.mExtents[1]; Index.mY++ )
      {
         for( Index.mZ=0; Index.mZ<mBlockCoord.mExtents[2]; Index.mZ++ )
         {
            ...
            //for(int i=2; i>-1; i--)
            {
                assert(((int)(Bounds.mMin[0]*100.0f))%((int)(mGranularities[0]*100.0f))==0);
                assert(((int)(Bounds.mMin[2]*100.0f))%((int)(mGranularities[2]*100.0f))==0);
               // assert(((int)(Bounds.mMax[i]*100.0f))%((int)(mGranularities[i]*100.0f))==0);
            }
