/*===========================================================================
    ...
    This tool splits every geo in the file into it's component primgroups, such that each primgroup has its own geo
            ...
            case Pure3D::Mesh::MESH:
                {
                    ...
                    tlPrimGroupMesh* pNewPGMesh = NULL;
                    
                    int nPGs = pPGMesh->NumPrimGroups();
                    char pPGName[255];

                    for( int i=nPGs-1; i>-1; i-- )
                    {
                        sprintf(pPGName, "%s_%03d", pPGMesh->GetName(), i );

                        tlPrimGroup* pPG = pPGMesh->GetPrimGroup(i);
                        tlPrimGroup* pPGClone = new tlPrimGroup(*pPG);

                        pNewPGMesh = new tlPrimGroupMesh();
                            pNewPGMesh->SetName(pPGName);
                            pNewPGMesh->AddPrimGroup(pPGClone);
                            pNewPGMesh->UpdateBounds();
                            pNewPGMesh->SetCastShadow(pPGMesh->CastShadow());
                        
                            outchunk->AppendSubChunk(pNewPGMesh->Chunk());
                        delete pNewPGMesh;
                    }
                    delete pPGMesh;
                }
