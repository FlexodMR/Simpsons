...
bool TransformMatrix::Assign(const MDagPath& dagPath)
{
    ...
    {
        MFnTransform xformNode(node, &status);
        if (!status)
            return false;
...
bool TransformMatrix::AssignJoint(MDagPath& dagPath, MObject& node)
{
    ...
    tmpVector = jointNode.translation(MSpace::kTransform, &status);
    if (!status)
        return false;
...
bool TransformMatrix::Assign(const MTransformationMatrix& xform)
{
    ...
    tmpPoint = xform.scalePivot(MSpace::kTransform, &status);
    if (!status)
        return false;
    ...
    status = xform.getScale(tmpArray, MSpace::kTransform);
    if (!status)
        return false;
    ...
    status = xform.getShear(tmpArray, MSpace::kTransform);
    if (!status)
        return false;
    ...
    tmpPoint = xform.scalePivotTranslation(MSpace::kTransform, &status);
    if (!status)
        return false;
    ...
    tmpPoint = xform.rotatePivot(MSpace::kTransform, &status);
    if (!status)
        return false;
    ...
    tmpVector = xform.rotatePivotTranslation(MSpace::kTransform, &status);
    if (!status)
        return false;
    ...
    tmpVector = xform.translation(MSpace::kTransform, &status);
    if (!status)
        return false;
    translate.x = (float)tmpVector.x;
    translate.y = (float)tmpVector.y;
