...
public:
    ...
    inline void Vertex(pddiVector* v, pddiColour c, pddiVector2* uv, float w);
#endif
...
d3dPrimBufferStream::d3dPrimBufferStream() :
    locked(false), basePtr(NULL), nVertex(0), maxVertex(0), stride(0), nColourSets( 0 ),
    coord(NULL), normal(NULL), binormal(NULL), tangent(NULL), compressedNormal(NULL), colour(NULL), specular(NULL), uv(NULL)
{
#ifdef _XBOX
    size = NULL;
    w = NULL;
...
void d3dPrimBufferStream::SetBasePtr(void* base, d3dVertexProgram* vs, int bufferCount, int offset)
{
    ...
    {
#ifdef _XBOX
        compressedNormal = (unsigned*)bufferPtr;
        bufferPtr += sizeof(unsigned);
#else
    ...
    {
        binormal = (float*)bufferPtr;
        bufferPtr += sizeof(float) * 3;
    }
    if(vertexMask & PDDI_V_TANGENT)
    {
        tangent = (float*)bufferPtr;
        bufferPtr += sizeof(float) * 3;
    }
#endif
}
...
void d3dPrimBufferStream::NextVertex()
{
    ...
    if(binormal)
        binormal = (float*)((char*)binormal + stride);

    if(tangent)
        tangent = (float*)((char*)tangent + stride);
...
#ifdef _XBOX
    if (size)
        size = (float*)((char*)size + stride);
    if (w)
        w = (float*)((char*)w + stride);
#endif
...
void d3dPrimBufferStream::Binormal(float x, float y, float z)
{
    binormal[0] = x;
    binormal[1] = y;
    binormal[2] = z;
...
void d3dPrimBufferStream::Tangent(float x, float y, float z)
{
    tangent[0] = x;
    tangent[1] = y;
    tangent[2] = z;
...
inline void d3dPrimBufferStream::Size(float s)
{
    *size = s;
}

inline void d3dPrimBufferStream::W(float val)
{
    *w = val;
}
...
void d3dPrimBufferStream::SkinWeights(float a, float b, float c)
{
    ...
    tmpx = (int(a * 0x7fff0000) >> 21) & (0xffffffff >> 21);
    tmpy = (int(b * 0x7fff0000) >> 21) & (0xffffffff >> 21);
    tmpy <<= 11;
    tmpz = (int(c * 0x7fff0000) >> 22) & (0xffffffff >> 22);
    tmpz <<= 22;

    *skinWeights = tmpz | tmpy | tmpx;
}
...
inline void d3dPrimBufferStream::Vertex(pddiVector* v, pddiColour c, float s)
{
    *coord = *v;
    *colour = c;
    *size = s;
...
inline void d3dPrimBufferStream::Vertex(pddiVector* v, pddiColour c, float s, float wVal)
{
    *coord = *v;
    *colour = c;
    *size = s;
...
inline void d3dPrimBufferStream::Vertex(pddiVector* v, pddiColour c, pddiVector2* t, float wVal)
{
    *coord = *v;
    *colour = c;
    uv[0] = *t;
...
void d3dPrimBuffer::SetNumIndices(int count)
{
    ...
    if(count)
    {
        HRESULT res = d3d->CreateIndexBuffer(count * sizeof(unsigned short), D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_MANAGED, &indices);
        DDVERIFY(res);
