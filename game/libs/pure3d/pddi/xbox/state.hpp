...
public:
    ...
    void SetMaterial(bool lit, bool ts, d3dShaderInfo* info)
    {
        if((twoSided != ts) || (isLit != lit))
        {
            ...
            #ifdef _XBOX_
//                d3d->SetRenderState(D3DRS_TWOSIDEDLIGHTING, lit);
            #endif
            }
            else
            {
            #ifdef _XBOX_
//                d3d->SetRenderState(D3DRS_TWOSIDEDLIGHTING, FALSE);
            #endif
                context->ResetCull();
            }
        }
...
protected:
    ...
    DWORD colourOp[NUM_STAGES];
    DWORD alphaOp[NUM_STAGES];
