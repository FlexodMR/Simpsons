...
void d3dShader::SetEmissiveAlpha(int alpha)
{
    if(alpha < 255)
    {
		//change diffuse alpha only when emssive alpha is not 255
    ...
    {
		shaderInfo.diffuse.SetAlpha(255);
...
void d3dShader::CopySettings( const d3dShader& right )
{
    context      = right.context;
    d3d          = right.d3d;
    blendMode    = right.blendMode;
    srcBlend     = right.srcBlend;
    destBlend    = right.destBlend;
    alphaTest    = right.alphaTest;
    alphaCompare = right.alphaCompare;
    alphaRef     = right.alphaRef;
    textureGen   = right.textureGen;
    uvMode       = right.uvMode;
    filterMode   = right.filterMode;
    isLit        = right.isLit;
    shadeMode    = right.shadeMode;
    shaderInfo   = right.shaderInfo;
    twoSided     = right.twoSided;
    skinMode     = right.skinMode;
}
