...
void d3dProjTextureShader2::PreRender(void)
{
    ...
    pddiTransposeMatrix( &projMatrix, &transpose );

    m_pd3dDevice->SetVertexShaderConstant(  0, &vZero,    1 );    
    m_pd3dDevice->SetVertexShaderConstant(  2, &transpose, 4 );    
    m_pd3dDevice->SetVertexShaderConstant(  6, &projVector,    1 );

    m_pd3dDevice->SetVertexShaderConstant(  10, &matWVPTranspose, 4 );    
