...
//-------------------------------------------------------
pddiShadeColourTable gcLayeredLMapShader::gColourTable[] = 
{
...
//-------------------------------------------------------
pddiShadeTextureTable gcLayeredLMapShader::gTextureTable[] = 
{
    {PDDI_SP_BASETEX,  SHADE_TEXTURE(&SetTexture)},
    {PDDI_SP_TOPTEX,   SHADE_TEXTURE(&SetTopTex)},
    {PDDI_SP_LIGHTMAP, SHADE_TEXTURE(&SetLightMapTex)},
    {PDDI_SP_NULL , NULL}
};

//-------------------------------------------------------
pddiShadeIntTable gcLayeredLMapShader::gIntTable[] = 
{
...
//-------------------------------------------------------
pddiShadeFloatTable gcLayeredLMapShader::gFloatTable[] = 
{
...
//-------------------------------------------------------
gcLayeredLMapShader::gcLayeredLMapShader(gcContext* c) 
{
    mContext = c;
...
//-------------------------------------------------------
gcLayeredLMapShader::~gcLayeredLMapShader() 
{
    if (mTexture != NULL)     mTexture->Release();
    if (mTopTex != NULL)      mTopTex->Release();
    if (mLightMapTex != NULL) mLightMapTex->Release();
}


//-------------------------------------------------------
void gcLayeredLMapShader::Install(void)
{
    pddiBaseShader::InstallShader("layeredlmap", &gcLayeredLMapShader::Allocate, NULL);
}

//-------------------------------------------------------
// this function is called by the base shader to allocate
// a new instance of a simple shader
pddiBaseShader* gcLayeredLMapShader::Allocate(pddiRenderContext *c, const char *name, const char *aux)
{
    return new gcLayeredLMapShader((gcContext*)c);
}

//-------------------------------------------------------
const char* gcLayeredLMapShader::GetType(void)
{
    static char layeredlmap[] = "layeredlmap";
    return layeredlmap;
}

//-------------------------------------------------------
int gcLayeredLMapShader::GetPasses(void)
{
    return 1;
}

//-------------------------------------------------------
void gcLayeredLMapShader::SetPass(int pass)
{
    SetDevPass((unsigned)pass);
}

//-------------------------------------------------------
void gcLayeredLMapShader::SetTexture(pddiTexture *t)
{
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetTopTex(pddiTexture* t)
{
    if(t == mTopTex) return;
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetLightMapTex(pddiTexture* t)
{
    if(t == mLightMapTex) return;
    
    mDirty = true;
    
    if(mLightMapTex != NULL) mLightMapTex->Release();

    mLightMapTex = (gcTexture *)t;
    if(mLightMapTex != NULL) mLightMapTex->AddRef();

}

//-------------------------------------------------------
void gcLayeredLMapShader::SetUVMode(int mode) 
{
    mDirty = true;
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetFilterMode(int mode) 
{
    mDirty = true;
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetTextureBlendMode(int mode)
{
    mDirty = true;
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetShadeMode(int shade) 
{
    mDirty = true;
...
//-------------------------------------------------------
void gcLayeredLMapShader::EnableLighting(int b)
{
    mDirty = true;
    mIsLit = (b != 0);
}
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetAmbient(pddiColour a) 
{
    mDirty = true;
    mAmbient = a;
}
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetDiffuse(pddiColour colour) 
{
    mDirty = true;
    mDiffuse = colour;
}
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetSpecular(pddiColour c) 
{
    mDirty = true;
    mSpecular = c;
}
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetEmissive(pddiColour c) 
{
    mDirty = true;
    mEmissive = c;
}
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetShininess(float power) 
{
    mDirty = true;
    mShininess = power;
}
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetBlendMode(int mode) 
{
    mDirty = true;
...
//-------------------------------------------------------
void gcLayeredLMapShader::EnableAlphaTest(int b) 
{
    mDirty = true;
    mAlphaTest = (b != 0);
}
...
//-------------------------------------------------------
void gcLayeredLMapShader::SetAlphaCompare(int compare) 
{
    mDirty = true;
...
//-------------------------------------------------------
int gcLayeredLMapShader::CountDevPasses(void) 
{
    return 1;
}

//-------------------------------------------------------
void gcLayeredLMapShader::SetDevPass(unsigned pass)
{
    Rebuild();
    ...
    if (mLightMapTex)
    {
        mLightMapTex->LoadTexture(GX_TEXMAP2);
        
        nNumTevStages++;
        nNumTexGens++;
        GXSetTexCoordGen((GXTexCoordID)nTexCoord, GX_TG_MTX2x4, GX_TG_TEX2, GX_IDENTITY);        
    }
...
//
//
void gcLayeredLMapShader::Rebuild(void)
{
    if (!mDirty) return;
    ...
    mTEVStates[3].mTEVSet = &TEVSet[set + 2];
    ...
    mTEVStates[2].mColourChannelID = GX_COLOR0A0;
    ...
    {
        // Enable top texture
        mTEVStates[2].mTextureUVID  = (GXTexCoordID)nTexCoord;
        mTEVStates[2].mTextureMapID = GX_TEXMAP2;
        mLightMapTex->SetWrapMode(gcUVWrapMode[mUVMode]);
    }
