...
--enter
 ...
 in_vf _LRGB2(VF23)
 in_vf _LRGB3(VF24)
 in_vf _VIEW(VF25) ; _SHININESS VF25w
...
TOP_SPEC_LIT:
    ...
    ; find the view point at infinity
    sub.xyz _VIEW , vf00, _VIEW            ; negate

    VectorNormalizeXYZ _VIEW, _VIEW

    ; find the half way vector between the viewer and the light
    ; H = ( L + V ) / || L + V ||

    add.x HalfVec , _VIEW, _LVX[x]
    add.y HalfVec , _VIEW, _LVY[x]
    add.z HalfVec , _VIEW, _LVZ[x]

    VectorNormalizeXYZ HalfVec, HalfVec

    sq.xyz HalfVec,0(vi00) ; force calculation
...
--LoopCS	3,0
    ...
    ibgtz VertCount,SPEC_LIT_LOOP      ; and loop
