...
)
{
	...
	else
	{
		...
		{
			printf( "Error: No default target found or specified on command line." );
			return( false );
		}                   
	}
    //
...
)
{
    ...
    switch( connectionState )
    {
        ...
        {   
            ...
            {
				m_pCallbackFunction( HrcsFail );;
                printf( "Error: Failed to attach to target \"%s\" because [%s].", m_Target , Message );    
            }
            break;
        }
        ...
        while( bytesReceived != 0 )
        {
            ...
            switch( *((HrcsCommand*) pRxAddress) )
            {
				...
				case HrcsResult :
				{
					...
					{
						printf( "Remote function call was not succesful.\n" );
					}
                    //
                    // Update receive pointer and bytes to process.
                    //
...
)
{
	...
	//
	printf( "ERROR: Unable to send to Remote Command Server\n" );	
	Terminate();
	}
}
